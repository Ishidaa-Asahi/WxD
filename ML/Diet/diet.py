...
def Weight_Loss():
    ...
    #print('1.bfid',breakfastfoodseparated)
    #print('lfid',LunchfoodseparatedID)
    #print('dfid' ,DinnerfoodseparatedID)
    ...
    print('LunchID1 : ',LunchfoodseparatedIDdata)
    ...
    for ii in range(len(y_pred)):
        if y_pred[ii]==2:     #weightloss
            ...
            final_data.append(Food_itemsdata[ii])
            if int(veg)==1:
    ...
    #print('findata: ', final_data)
    print("###############################################")
    
    brk_foods = []
    print('Suggested Breakfast food:')
    for i in final_data:
        if i in breakfastfoodseparated :
            print(i)
            brk_foods.append(i)
    print('brkfooods: ',brk_foods)
    
    lunch_foods = []
    print("###############################################")
    print('Suggested Lunch food:')
    for i in final_data:
        if i in Lunchfoodseparated:
            print(i)
            lunch_foods.append(i)
            
    din_foods = []
    print("###############################################")        
    print('Suggested Dinner food:')
    for i in final_data:
        if i in Dinnerfoodseparated :
            print(i)
            din_foods.append(i)
    ...
    brk_cal_vals = []
    for i in range(len(brk_foods)):
        j = brk_foods[i]
        indexes = int(data[data['Food_items']==j].index[0])
        cals_indexes = data._get_value(indexes, 'Calories')
        brk_cal_vals.append(cals_indexes)
    brk_cal_avg = sum(brk_cal_vals)/len(brk_cal_vals)
    print(brk_cal_avg)
    
    lunch_cal_vals = []
    for i in range(len(lunch_foods)):
        j = lunch_foods[i]
        indexes = int(data[data['Food_items']==j].index[0])
        cals_indexes = data._get_value(indexes, 'Calories')
        lunch_cal_vals.append(cals_indexes)
    lunch_cal_avg = sum(lunch_cal_vals)/len(lunch_cal_vals)
    print(lunch_cal_avg)
    
    din_cal_vals = []
    for i in range(len(din_foods)):
        j = din_foods[i]
        indexes = int(data[data['Food_items']==j].index[0])
        cals_indexes = data._get_value(indexes, 'Calories')
        din_cal_vals.append(cals_indexes)
    din_cal_avg = sum(din_cal_vals)/len(din_cal_vals)
    print(din_cal_avg)
    
    cal_perday = (brk_cal_avg + lunch_cal_avg + din_cal_avg)/3
    print('cal_perday: ',cal_perday)
